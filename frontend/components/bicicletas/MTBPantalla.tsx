...
const MTBPantalla: React.FC = () => {
  ...
  const buscarArticulos = async (texto: string) => {
    ...
    try {
      ...
      const resultados: Articulo[] = Array.isArray(data) ? data : Array.isArray(data.articulos) ? data.articulos : [];
            ...
            {cargando ? (
              ...
              <View style={{ paddingHorizontal: 16, paddingBottom: 20 }}>
                <Text style={{ textAlign: 'center', fontSize: 16, lineHeight: 22, color: '#fff', fontWeight: '500', paddingHorizontal: 16 }}>
                  La bicicleta de montaña (MTB) está diseñada para senderos y terrenos difíciles, ofreciendo resistencia, tracción y control en condiciones extremas.
                </Text>
...
const styles = StyleSheet.create({
  ...
  // Botones
  botonCarrito: { flexDirection:'row', alignItems:'center', justifyContent:'center', backgroundColor:'#5bdaedff', paddingVertical:8, borderRadius:10, marginTop:8 },
  textoCarrito: { color:'#fff', fontSize:14, fontWeight:'600', marginLeft:6 },
  botonWhatsapp: { flexDirection:'row', alignItems:'center', justifyContent:'center', backgroundColor:'#25D366', paddingVertical:8, borderRadius:10, marginTop:8 },
